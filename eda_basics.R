...
df_sample <- diamonds[sample(nrow(diamonds),5000),]
...
# identify outliers using boxplots # scaled the data first
...
df_new$price <- log10(df_new$price) # this will scale the values in a range
# shahroz ka code
new_df_clean <- df_new %>% 
  filter(!(cut == "Fair" & price < 2.7))

box_plot <- ggplot(new_df_clean, aes(x = cut, y = price, fill = cut)) +
  geom_boxplot() + 
  labs(title = "Box Plot of Price by Cut", x = "Cut", y = "Price") +
  theme_minimal()

#data_clean <- fair_diamonds[!(fair_diamonds$price < (q1 - 1.5 * iqr) | fair_diamonds$price > (q3 + 1.5 * iqr)), ]
...
df_new$xyz <- df_new$x + df_new$y + df_new$z
...
ggplot(df_new, aes(x = price)) + 
  geom_histogram()
ks.test(df_new$price, "pnorm")
# plot the distribution of a variable
ggplot(data, aes(x = column_name)) + 
  geom_density()

ggplot(df_new, aes(x = price, fill = cut)) + 
  geom_density()
...
# not normalized
ggplot(df, aes(y = price, fill = df$cut)) +
  geom_boxplot() 

# normalized
ggplot(df_new, aes(y = price, fill = df_new$cut)) +
  geom_boxplot() 
...
ggplot(df_new, aes(x = x, y = y)) + geom_point()

# explore the relationship between two variables using a scatter plot
ggplot(data, aes(x = column1, y = column2)) + geom_point() + geom_smooth(method = "lm")

ggplot(df_new, aes(x = x, y = y)) + geom_point() + geom_smooth(method = "lm", se = FALSE) + ylim(0,10)
...
plot_ly(data, x = ~depth, y = ~price, type = "scatter", mode = "markers", color = ~cut)
